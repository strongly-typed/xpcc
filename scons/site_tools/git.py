...
def git_config(key):
	...
	"""
	try:
		return subprocess.check_output(['git', 'config', key], universal_newlines=True).split('\n', 1)[0]
...
def git_info_header(env):
	...
	try:
		...
		# Status
		s = subprocess.check_output(['git', '--no-pager', 'status', '--porcelain'], universal_newlines=True).split('\n')
